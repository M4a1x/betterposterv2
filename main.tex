% main.tex
% Compile with LuaLaTeX (recommended) or XeLaTeX
\documentclass[%
    colorscheme=purple,   % Options: default, green, blue, red, purple
    fontsize=42pt,        % Options: 36pt, 42pt, 48pt, ...
    papersize=a0paper,    % Options: a0paper, a1paper, a2paper, a3paper, a4paper, letterpaper, ...
    orientation=portrait, % Options: portrait, landscape
    debug=false           % Options: true, false
]{betterposterv2}

% --- Poster Layout (optional overrides) ---
% Customize poster geometry (defaults shown):
% \SetHeaderHeight{20cm}
% \SetFooterHeight{10cm}
% \SetSideMargin{3cm}

% --- Color Customization (optional) ---
% Override individual colors (defaults use colorscheme option):
% \SetPageColor{bpLightGrey}
% \SetHeaderColor{bpDark}
% \SetFooterColor{bpDark}
% \SetLightBoxColor{bpWhite}
% \SetDarkBoxColor{bpMediumGrey}
% \SetHeaderTextColor{white}
% \SetFooterTextColor{white}

% --- Font Size Customization (optional) ---
% Customize font sizes (defaults shown):
% \SetMainFindingSize{\Huge}
% \SetTitleSize{\Large}
% \SetBoxTitleSize{\large}
% \SetSectionSize{\Large}

% --- Poster Content Configuration ---
\mainfinding{A \$50 Open-Source NMR Spectrometer achieves \unit{\hertz}-level SDR and Python processing.}
\title{magnETHical: A Software-Defined Radio Approach to Low-Field NMR Spectroscopy}

% --- Footer Configuration ---
\qrcode{graphics/magnethical.eps}
\authors{Max Stabel, Roland Riek}  % if multiple affiliations, use \textsuperscript{}
\affiliations{D-CHAB, ETH Zürich, Switzerland}
\logo{graphics/eth.eps}
\publicationyear{2026}
\portrait{graphics/portrait.jpg}

\begin{document}

\maketitle

% --- BLOCK 1: BACKGROUND (Wide Grey) ---
\begin{bpbox}[bpcolor=grey, bpstyle=wide, bptitlebg=transparent, title={Background}]
    Current commercial NMR spectrometers cost upwards of \$50k, limiting access in education and developing regions. We propose a \SI{25}{\mega\hertz} (\SI{0.58}{\tesla}) permanent magnet system using commercial-off-the-shelf (COTS) RF components and a custom open-source Python analysis stack to democratize magnetic resonance.
\end{bpbox}

% --- BLOCK 2: METHODS & SYSTEM (Two Columns) ---
\bpsection{System Design}
\begin{bpcolumns}[2]
    \begin{bpbox}[title={Hardware Architecture}]
        The system relies on a Red Pitaya FPGA board acting as a Software Defined Radio (SDR).

        \begin{center}
            \includegraphics[width=\linewidth, height=8cm, keepaspectratio]{example-image-a}%
            \captionof{figure}{RF Signal Chain}
        \end{center}
    \end{bpbox}
    \begin{bpbox}[title={Python Processing Stack}]
        Data acquisition and processing are handled purely in Python, utilizing \texttt{numpy} and \texttt{scipy}.

        \textbf{Pulse Sequence:}
        A standard CPMG sequence is generated digitally.

        \begin{verbatim}
def process_signal(raw_data):
    # Downconvert
    iq = raw_data * np.exp(-1j * w_c * t)
    # Decimate
    return scipy.signal.decimate(iq, 10)
        \end{verbatim}
    \end{bpbox}
\end{bpcolumns}

% --- BLOCK 3: RESULTS ---
\bpsection{Results}
\begin{bpbox}[title={Results Overview}]
    \begin{minipage}{0.45\textwidth}
        \textbf{Ethanol Spectrum Analysis:}\\
        The figure on the right demonstrates a single-shot acquisition of an Ethanol sample.

        We observe clearly resolved J-coupling in the methyl triplet and methylene quartet. The linewidth (FWHM) is approximately \textbf{\SI{4.5}{\hertz}} after shimming.
    \end{minipage}%
    \hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=12cm, keepaspectratio]{example-image-b}%
        % \captionof{figure}{\SI{25}{\MHz} \textsuperscript{1}H-NMR spectrum of Ethanol.}
    \end{minipage}
\end{bpbox}

% --- BLOCK 4: LIMITATION (Wide Grey) ---
\begin{bpbox}[bpcolor=grey, bpstyle=wide, title={Limitation}]
    Field homogeneity remains the primary challenge. Thermal drift of the permanent magnet shifts the Larmor frequency by approx \SI{200}{\hertz\per\degreeCelsius}, requiring an external $B_0$ lock loop or temperature stabilization chamber for long-term averaging.
\end{bpbox}

\end{document}